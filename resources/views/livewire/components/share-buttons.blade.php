...
<div class="flex items-center space-x-4">
    ...
    {{-- <button wire:click="copyLink"
            class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors relative">
        ...
        <div x-data="{ show: false }"
             x-show="show"
             x-init="
                $wire.on('copyToClipboard', async ({ text }) => {
                    await navigator.clipboard.writeText(text);
                    show = true;
                });
                $wire.on('hideCopiedMessage', () => {
                    setTimeout(() => {
                        show = false;
                    }, 2000);
                });"
             class="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-sm px-2 py-1 rounded-sm shadow-lg"
...
<script>
    // Add this to your main JavaScript file or within a script tag in your layout
    document.addEventListener('alpine:init', () => {
        window.Alpine.data('shareButtons', () => ({
